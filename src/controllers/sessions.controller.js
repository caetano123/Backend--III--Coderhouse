...
const register = async (req, res) => {
  try {
    ...
    // DEVUELVE TODO EL OBJETO DEL USUARIO
...
const login = async (req, res) => {
  try {
    ...
    res.cookie('coderCookie', token, { maxAge: 3600000 }).status(200).send({
      status: "success",
      message: "Logged in",
      payload: userDto
    });
...
const unprotectedLogin = async (req, res) => {
  try {
    ...
    const token = jwt.sign({ _id: user._id, email: user.email }, 'tokenSecretJWT', { expiresIn: "1h" });

    res.cookie('unprotectedCookie', token, { maxAge: 3600000 }).status(200).send({
      status: "success",
